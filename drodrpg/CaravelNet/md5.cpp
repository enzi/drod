//ATTN: This code is not to be publicly released -- mrimer.
...
static void Permute( uint32 ABCD[4], const uchar block[64] )
  /* ------------------------------------------------------------------------ **
   ...
   * The uint32 values are then handled in host order.
   */
  for( i = 0, j = 0; i < 16; i++ )
    {
    X[i]  =  (uint32)block[j++];
    X[i] |= ((uint32)block[j++] << 8);
    X[i] |= ((uint32)block[j++] << 16);
    X[i] |= ((uint32)block[j++] << 24);
    }
...
auth_md5Ctx *auth_md5CloseCtx( auth_md5Ctx *ctx, uchar *dst )
  ...
  /* Add the total length and perform the final permutation.
...
{
	static char hexConversion[] = "0123456789abcdef";
	static char sumHex[33]; //to allow convenient returns
	uchar sum[17];

	auth_md5Sum(sum, (const uchar*)text.c_str(), text.length());

	//Convert 16-bit hex --> text.
	int count=0;
	for (int i=0; i<16; ++i)
	{
		sumHex[count++] = hexConversion[sum[i] / 0x10];
		sumHex[count++] = hexConversion[sum[i] % 0x10];
	}
	sumHex[32] = '\0';
	return sumHex;
